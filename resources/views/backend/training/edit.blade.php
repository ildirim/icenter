		...
		<div class="but_list">
			<div class="bs-example bs-example-tabs" role="tabpanel" data-example-id="togglable-tabs">
				...
				<div id="myTabContent" class="tab-content">
					<form class="form-horizontal" action="{{ route('trainings.update', $training->id) }}" method="post" enctype="multipart/form-data">
						<div class="tab-content">
							...
							<div class="tab-pane fade in @if($loop->iteration == 1) active @else  @endif" id="home{{$loop->iteration}}">
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="text" name="title[{{$key}}]" class="form-control" value="{{ $training->translate($key)->title }}">
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-2 control-label">{{ __('backend.content') }}</label>
									<div class="col-sm-9">
										<textarea name="content[{{$key}}]" class="form-control" rows="5">{{ $training->translate($key)->content }}</textarea>
									</div>
								</div>
							</div>
							@endforeach

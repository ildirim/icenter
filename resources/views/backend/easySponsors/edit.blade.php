		...
		<div class="but_list">
			<div class="bs-example bs-example-tabs" role="tabpanel" data-example-id="togglable-tabs">
				...
				<div id="myTabContent" class="tab-content">
					<form class="form-horizontal" action="{{ route('easySponsors.update', $easysponsor->id) }}" method="post" enctype="multipart/form-data">
						<div class="tab-content">
							...
							<div class="tab-pane fade in @if($loop->iteration == 1) active @else  @endif" id="home{{$loop->iteration}}">
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="text" name="title[{{$key}}]" class="form-control" value="{{ $easysponsor->translate($key)->title }}">
									</div>
								</div>
							</div>
							@endforeach
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								...
								@if($easysponsor->image)
								<a style="font-size: 16px" target="_blank" href="/storage/services/{{ $easysponsor->image }}">{{ __('backend.show') }}</a>
								@endif
							</div>
						</div>
						<div class="form-group">

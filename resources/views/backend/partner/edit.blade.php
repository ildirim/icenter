					...
					<form class="form-horizontal" action="{{ route('partners.update', $partner->id) }}" method="post" enctype="multipart/form-data">
						<div class="tab-content">
							...
							<div class="tab-pane fade in @if($loop->iteration == 1) active @else  @endif" id="home{{$loop->iteration}}">
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="text" name="title[{{$key}}]" class="form-control" value="{{ $partner->translate($key)->title }}">
									</div>
								</div>
							</div>
							@endforeach
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								...
								@if($partner->image)
								<a style="font-size: 16px" target="_blank" href="/storage/about/{{ $partner->image }}">{{ __('backend.show') }}</a>
								@endif
							</div>
						</div>
						<div class="form-group">

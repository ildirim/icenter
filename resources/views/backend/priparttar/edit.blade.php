		...
		<div class="but_list">
			<div class="bs-example bs-example-tabs" role="tabpanel" data-example-id="togglable-tabs">
				...
				<div id="myTabContent" class="tab-content">
					<form class="form-horizontal" action="{{ route('wsaPriPartTar.update', $priparttar->id) }}" method="post" enctype="multipart/form-data">
						<div class="tab-content">
							...
							<div class="tab-pane fade in @if($loop->iteration == 1) active @else  @endif" id="home{{$loop->iteration}}">
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="text" name="title[{{$key}}]" class="form-control" value="{{ $priparttar->translate($key)->title }}">
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-2 control-label">{{ __('backend.content') }}</label>
									<div class="col-sm-9">
										<textarea name="content[{{$key}}]" class="form-control" rows="5">{{ $priparttar->translate($key)->content }}</textarea>
									</div>
								</div>
						...
						@if(!is_null($priparttar->image))
						<div class="form-group">
							<label class="col-md-2 control-label">{{ __('backend.image') }}</label>
							<div class="col-sm-9">
								<input type="file" name="image" class="btn btn-default">
								@if($priparttar->image)
								<a style="font-size: 16px" target="_blank" href="/storage/wsa/{{ $priparttar->image }}">{{ __('backend.show') }}</a>
								@endif
							</div>
						</div>
						@endif
